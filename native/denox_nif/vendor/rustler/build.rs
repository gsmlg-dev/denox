...
fn write_fn_type(out: &mut String, args: &str, ret: &str) {
    write!(out, "unsafe extern \"C\" fn ({})", args).unwrap();
    write_ret(out, ret);
}
...
pub struct StaticSymbolsApiBuilder<'a>(&'a mut String);

impl ApiBuilder for StaticSymbolsApiBuilder<'_> {
    fn init(&mut self) {
        writeln!(self.0, "#[cfg(feature = \"static_nif\")]").unwrap();
    ...
    fn func(&mut self, ret: &str, name: &str, args: &str) {
        ...
        write!(self.0, "    fn static_{}({})", name, args).unwrap();
        write_ret(self.0, ret);
        writeln!(self.0, ";").unwrap();
    }
...
pub struct StaticWriterBuilder<'a>(&'a mut String);

impl ApiBuilder for StaticWriterBuilder<'_> {
    fn init(&mut self) {
        writeln!(
            self.0,
            "#[cfg(feature = \"static_nif\")]\nimpl DynNifCallbacks {{\n    fn write_static_symbols(&mut self) {{"
        )
        .unwrap();
    }
    ...
    fn func(&mut self, _ret: &str, name: &str, _args: &str) {
        writeln!(self.0, "        self.{name} = Some(static_{name});").unwrap();
    }
...
fn generate(opts: &GenerateOptions) -> String {
    ...
    build_api(&mut StaticSymbolsApiBuilder(&mut out), opts);

    build_api(&mut ForwardersApiBuilder(&mut out), opts);
    build_api(&mut WriterBuilder(&mut out), opts);
    build_api(&mut StaticWriterBuilder(&mut out), opts);
