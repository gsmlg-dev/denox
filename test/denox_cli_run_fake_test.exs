defmodule DenoxCLIRunFakeTest do
  @moduledoc """
  Tests for CLIRun using a fake shell script as the "deno" binary.
  ...
  describe "init_backend — success path" do
    test "opens a port and captures output" do
      ...
      {:ok, pid} = CLIRun.start_link(file: "test.ts", permissions: :all)

      # Covers init_backend success, handle_info {:eol, line}, dispatch_line
      assert {:ok, "hello from fake deno"} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
    ...
    test "os_pid returns an integer" do
      ...
      assert {:ok, os_pid} = CLIRun.os_pid(pid)
  ...
  describe "send_backend" do
    test "sends data to the port stdin" do
      ...
      :ok = CLIRun.send(pid, "hello\n")
      assert {:ok, line} = CLIRun.recv(pid, timeout: 5000)
      assert line =~ "got:"
    ...
    test "returns {:error, :closed} after process has exited" do
      ...
      assert {:error, :closed} = CLIRun.send(pid, "too late")
    end
  end
  ...
  describe "permissions_to_args" do
    test "covers :none permissions" do
      ...
      {:ok, pid} = CLIRun.start_link(file: "test.ts", permissions: :none)
    ...
    test "covers nil permissions" do
      ...
      {:ok, pid} = CLIRun.start_link(file: "test.ts", permissions: nil)
      {:ok, _} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
    ...
    test "covers {key, true} permission flag" do
      ...
      {:ok, pid} =
        CLIRun.start_link(file: "test.ts", permissions: [allow_net: true])
    ...
    test "covers {key, list_of_values} permission flag" do
      ...
      {:ok, pid} =
        CLIRun.start_link(file: "test.ts", permissions: [allow_env: ["FOO", "BAR"]])
    ...
    test "covers {key, false} permission flag" do
      ...
      {:ok, pid} =
        CLIRun.start_link(file: "test.ts", permissions: [allow_net: false])
  ...
  describe "resolve_specifier" do
    test "covers @ prefix → npm: resolution" do
      ...
      {:ok, pid} = CLIRun.start_link(package: "@scope/pkg", permissions: :all)
      {:ok, _} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
    ...
    test "covers bare specifier passthrough" do
      ...
      {:ok, pid} = CLIRun.start_link(package: "bare-pkg", permissions: :all)
      {:ok, _} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
  end
  ...
  describe "resolve_specifier — file:// prefix" do
    test "passes file:// specifier through unchanged" do
      ...
      {:ok, pid} = CLIRun.start_link(package: "file://test.ts", permissions: :all)
      {:ok, _} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
    ...
    test "passes npm: specifier through unchanged" do
      ...
      {:ok, pid} = CLIRun.start_link(package: "npm:pkg@1.0", permissions: :all)
      {:ok, _} = CLIRun.recv(pid, timeout: 5000)
      CLIRun.stop(pid)
    end
  end
  ...
  describe "error paths" do
    test "raises ArgumentError for unknown permission flag" do
      ...
      assert {:error, {%ArgumentError{message: msg}, _}} =
               CLIRun.start_link(file: "test.ts", permissions: [unknown_perm: true])
