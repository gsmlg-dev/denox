defmodule Denox.StaticNifReleaseTest do
  ...
  test "release workflow builds and verifies static archives against musl link hazards" do
    ...
    assert build_script =~ "registry/cache"
    assert build_script =~ "registry/src"
    assert build_script =~ "unpack_registry_crate 'v8-*'"
    assert build_script =~ "unpack_registry_crate 'deno_core_icudata-*'"
    assert build_script =~ "tar -xzf \"${crate_file}\""
